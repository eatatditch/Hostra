...
- [x] **Day 4 · no-show-fees** — In `markNoShow`, if there's a linked payment with status `requires_capture`, capture it. Surface captured fee in reservation detail and guest profile.
- [x] **Day 5 · cancellation-policy** — Per-location `cancellation_window_hours` and `cancellation_refund_percent`. On `cancelReservation` inside window, refund pending deposit at percent. Expose policy on the guest booking confirmation page.
